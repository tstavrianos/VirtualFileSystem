    ...
    public readonly struct UPath: IEquatable<UPath>, IComparable<UPath>
    {
        ...
        public bool IsAbsolute
        {
            get
            {
                ...
                if (!IsOnWindows) return this.FullName.StartsWith('/');
                if (this.FullName.Length < 3) return false;
                return IsDriveLetter(this.FullName[0])  && this.FullName[1] == ':' && this.FullName[2] == '/';
            }
        }
        ...
        public UPath(string value): this(value, false)
        {
        }

        internal UPath(string value, bool safe)
        {
            if (safe)
            {
                this.FullName = value;
            ...
            {
                this.FullName = ValidateAndNormalize(value, out var errorMessage);
            ...
            try
            {
                ...
                for (i = 0; i < parts.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        // Throws an exception if our slice part contains only `.`  and is longer than 2 characters
                        if (partLength > 2)
                        {
                            var isValid = false;
        ...
        public string Name => this.IsNull ? null : Path.GetFileName(this.FullName);
        
        public bool IsInDirectory(UPath directory, bool recursive)
        {
            this.AssertNotNull();
            ...
            var dirHasTrailingSeparator = dir[^1] == DirectorySeparator;

            if (!recursive)
            {
                // need to check if the directory part terminates 
                var lastSeparatorInTarget = target.LastIndexOf(DirectorySeparator);
            ...
            if (!dirHasTrailingSeparator)
            {
                ...
                return target.Length > dir.Length && target[dir.Length] == DirectorySeparator;
            }

            return true;
        }
        
        public UPath AssertNotNull(string name = "path")
        {
